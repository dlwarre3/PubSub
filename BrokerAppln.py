...
class BrokerAppln ():
  ...
  def __init__ (self, logger):
    ...
    self.topiclist = None # the different topics that we publish on -- THIS WILL BE ALL OF THEM
    self.iters = None   # number of iterations of publication
    self.frequency = None # rate at which dissemination takes place
    self.num_topics = 9 # total num of topics we publish -- AGAIN WE PUBLISH ALL TOPICS
    self.lookup = None # one of the diff ways we do lookup
    self.dissemination = None # direct or via broker
    self.publist = set() # set of all publishers 
  ...
  def invoke_operation (self):
    ...
    try:
      ...
      if (self.state == self.State.REGISTER):
        ...
        self.logger.debug("BrokerAppln::invoke_operation - lookup all publishers")
        self.mw_obj.lookup_allpublishers (self.topiclist)
        ...
        if (self.dissemination == "Broker"):
          for i in range (self.iters):
            ...
            # about their values. But in future assignments, this can change.
              self.mw_obj.disseminate (self.name, self.topiclist, self.publist)
  ...
  def isready_response (self, isready_resp):
    ...
    try:
      self.logger.info ("BrokerAppln::isready_response" + str(isready_resp))
      ...
      if not isready_resp.status:
        ...
        self.logger.debug ("BrokerAppln::driver - Status success. Ready to disseminate.")
  ...
  ########################################
  # handle lookup_allpublishers response method called as part of upcall
  #
  # Also a part of upcall handled by application logic
  ########################################
  def lookup_allpublishers_response (self, lookup_all_resp):
    ''' handle isready response '''

    try:
      self.logger.info ("BrokerAppln::isready_response" + str(lookup_all_resp))

      # Notice how we get that loop effect with the sleep (10)
      # by an interaction between the event loop and these
      # upcall methods.
      if not lookup_all_resp:
        # discovery service is not ready yet
        self.logger.debug ("BrokerAppln::driver - Not ready yet; check again")
        time.sleep (10)  # sleep between calls so that we don't make excessive calls
        ...
        self.logger.debug ("BrokerAppln::driver - publishers found - " + str(lookup_all_resp))
        for item in lookup_all_resp.info:
          self.logger.debug ("BrokerAppln::driver - adding " + str(item.addr) + ":" + str(item.port) + " to pub list")
          self.publist.add((item.addr, item.port))
