...
class SubscriberAppln ():
  ...
  def __init__ (self, logger):
    ...
    self.iters = None   # number of iterations of subcription
    self.frequency = None # rate at which we consume
    self.num_topics = None # total num of topics we subscribe to
    self.lookup = None # one of the diff ways we do lookup
    self.dissemination = None # direct or via broker
    self.lookupResult = set() # publishers (or broker) we are interested in
  ...
  def configure (self, args):
    ...
    try:
      ...
      self.num_topics = args.num_topics  # total num of topics we subscribe to
      ...
      # format the logger for time series data
      fh = logging.FileHandler(self.name + '.csv')
...
########################################
  def invoke_operation (self):
    ...
    try:
      ...
      if (self.state == self.State.REGISTER):
        ...
        self.logger.debug("SubscriberAppln::invoke_operation - look for matching topics")
        if (self.dissemination == "Direct"):
          self.mw_obj.lookup_publishers(self.topiclist)
        elif (self.dissemination == "Broker"):
        ...
        # left for us as a subscriber is consuming data, which we do it actively here.
        self.logger.debug ("SubscriberAppln::invoke_operation - start Consuming")

        for i in range (self.iters):
          for item in self.lookupResult:
            self.mw_obj.consume (self.name, self.topiclist, item[0], item[1])

        self.logger.debug ("SubscriberAppln::invoke_operation - finished Consuming")
      ...
      self.logger.info ("Subscriberln::invoke_operation completed")
    except Exception as e:
      raise e
  ...
  def isready_response (self, isready_resp):
    ...
    try:
      ...
      if not isready_resp.status:
        ...
        self.logger.debug ("SubscriberAppln::driver - Ready to find publishers")
  ...
  ########################################
  # handle lookup response method called as part of upcall
  #
  # Also a part of upcall handled by application logic
  ########################################
  def lookup_response (self, lookup_resp):
    ...
    try:
      ...
      # upcall methods.
      if not lookup_resp:
        # no publishers of interest available
        self.logger.debug ("SubscriberAppln::driver - No interesting publishers found; check again")
        time.sleep (10)  # sleep between calls so that we don't make excessive calls

      else:
        # we got the go ahead
        # set the state to consume
        self.logger.debug ("SubscriberAppln::driver - publisher found - " + str(lookup_resp))
        if (self.dissemination == "Direct"):
          for item in lookup_resp.info:
            self.lookupResult.add((item.addr, item.port))
          self.logger.debug ("SubscriberAppln::driver - Ready to consume from - " + str(self.lookupResult))
        elif (self.dissemination == "Broker"):
          self.lookupResult.add((lookup_resp.info.addr, lookup_resp.info.port))
...
def parseCmdLineArgs ():
  ...
  parser.add_argument ("-a", "--addr", default="127.0.0.1", help="IP addr of this publisher to advertise (default: localhost)")
  ...
  parser.add_argument ("-d", "--discovery", default="10.0.0.1:5555", help="IP Addr:Port combo for the discovery service, default localhost:5555")
