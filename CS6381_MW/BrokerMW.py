...
class BrokerMW ():
  ...
  def __init__ (self, logger):
    ...
    self.sub = None # will be a ZMQ PUB socket for dissemination
  ...
  def configure (self, args):
    ...
    try:
      ...
      # PUB is needed because we publish topic data (from the anonymous publishers)
      # SUB is needed because we subscribe to all publisherse
  ...
  def handle_reply (self):
    ...
    try:
      ...
      if (disc_resp.msg_type == discovery_pb2.TYPE_REGISTER):
        ...
        timeout = self.upcall_obj.lookup_allpublishers_response (disc_resp.lookup_all_resp)
  ...
  def register (self, name, topiclist):
    ...
    try:
      ...
      disc_req.msg_type = discovery_pb2.TYPE_REGISTER  #PublisherMWset message type
      # It was observed that we cannot directly assign the nested field here.
      # A way around is to use the CopyFrom method as shown
  ...
  ########################################
  def lookup_allpublishers (self, topiclist):
    ''' lookup all publishers '''

    try:
      self.logger.info ("BrokerMW::lookup_publishers")
      ...
      # first build a IsReady message
      self.logger.debug ("BrokerMW::lookup_publishers - populate the nested LookupPubByTopicReq msg")
      pub_req = discovery_pb2.LookupAllPubsReq ()  # allocate 
      self.logger.debug ("BrokerMW::lookup_publishers - done populating nested LookupPubByTopicReq msg")

      # Build the outer layer Discovery Message
      self.logger.debug ("BrokerMW::lookup_publishers - build the outer DiscoveryReq message")
      disc_req = discovery_pb2.DiscoveryReq ()
      disc_req.msg_type = discovery_pb2.TYPE_LOOKUP_ALL_PUBS
      # It was observed that we cannot directly assign the nested field here.
      # A way around is to use the CopyFrom method as shown
      disc_req.lookup_all_req.CopyFrom (pub_req)
      self.logger.debug ("BrokerMW::lookup_publishers - done building the outer message")
      ...
      # now send this to our discovery service
      self.logger.debug ("BrokerMW::lookup_publishers - send stringified buffer to Discovery service")
      self.req.send (buf2send)  # we use the "send" method of ZMQ that sends the bytes
      
      # now go to our event loop to receive a response to this request
      self.logger.info ("BrokerMW::lookup_publishers - request sent and now wait for reply")
  ...
  #################################################################
  def disseminate (self, id, topiclist, publist):
    try:
      ...
      for pub in publist:
        self.sub.connect("tcp://" + str(pub[0]) + ":" + str(pub[1]))
      ...
      send_str = self.sub.recv()
      self.logger.info ("BrokerMW::disseminate - captured {}".format (send_str))

      # send the info as bytes. See how we are providing an encoding of utf-8
      self.pub.send (send_str)
