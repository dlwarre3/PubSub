...
class SubscriberMW ():
  ...
  def __init__ (self, logger):
    ...
    self.port = None # port num where we are going to consume our topics
  ...
  def configure (self, args):
    ...
    try:
      ...
      # SUB is needed because we consume topic data
      self.logger.debug ("SubscriberMW::configure - obtain REQ and SUB sockets")
      self.req = context.socket (zmq.REQ)
      self.sub = context.socket (zmq.SUB)

      # Since are using the event loop approach, register the REQ socket for incoming events and
      # SUB socket to consume data
  ...
  def handle_reply (self):
    ...
    try:
      ...
      if (disc_resp.msg_type == discovery_pb2.TYPE_REGISTER):
        ...
        # this is a response to lookup request
        timeout = self.upcall_obj.lookup_response (disc_resp.broker_resp)
  ...
  ########################################
  # ask the discovery service for a list of publishers
  # that are publishing topics of interest
  ########################################
  def lookup_publishers (self, topiclist):
    ...
    try:
      ...
      # first build a IsReady message
      self.logger.debug ("SubscriberMW::lookup_publishers - populate the nested LookupPubByTopicReq msg")
      pub_req = discovery_pb2.LookupPubByTopicReq ()  # allocate 
      pub_req.topiclist.extend(topiclist)
      self.logger.debug ("SubscriberMW::lookup_publishers - done populating nested LookupPubByTopicReq msg")

      # Build the outer layer Discovery Message
      self.logger.debug ("SubscriberMW::lookup_publishers - build the outer DiscoveryReq message")
      disc_req = discovery_pb2.DiscoveryReq ()
      disc_req.msg_type = discovery_pb2.TYPE_LOOKUP_PUB_BY_TOPIC
      # It was observed that we cannot directly assign the nested field here.
      # A way around is to use the CopyFrom method as shown
      disc_req.lookup_req.CopyFrom (pub_req)
      self.logger.debug ("SubscriberMW::lookup_publishers - done building the outer message")
      ...
      # now send this to our discovery service
      self.logger.debug ("SubscriberMW::lookup_publishers - send stringified buffer to Discovery service")
      self.req.send (buf2send)  # we use the "send" method of ZMQ that sends the bytes
      
      # now go to our event loop to receive a response to this request
      self.logger.info ("SubscriberMW::lookup_publishers - request sent and now wait for reply")
  ...
  # that are publishing topics of interest
  ########################################
  def lookup_broker (self, topiclist):
    ''' register the appln with the discovery service '''

    try:
      self.logger.info ("SubscriberMW::lookup_broker")
      ...
      # first build a IsReady message
      self.logger.debug ("SubscriberMW::lookup_broker - populate the nested LookupBrokerReq msg")
      broker_req = discovery_pb2.LookupBrokerReq ()  # allocate 
      self.logger.debug ("SubscriberMW::lookup_broker - done populating nested LookupBrokerReq msg")

      # Build the outer layer Discovery Message
      self.logger.debug ("SubscriberMW::lookup_broker - build the outer DiscoveryReq message")
      disc_req = discovery_pb2.DiscoveryReq ()
      disc_req.msg_type = discovery_pb2.TYPE_LOOKUP_BROKER
      # It was observed that we cannot directly assign the nested field here.
      # A way around is to use the CopyFrom method as shown
      disc_req.broker_req.CopyFrom (broker_req)
      self.logger.debug ("SubscriberMW::lookup_broker - done building the outer message")
      ...
      # now send this to our discovery service
      self.logger.debug ("SubscriberMW::lookup_broker - send stringified buffer to Discovery service")
      self.req.send (buf2send)  # we use the "send" method of ZMQ that sends the bytes
      
      # now go to our event loop to receive a response to this request
      self.logger.info ("SubscriberMW::lookup_broker - request sent and now wait for reply")
      
    except Exception as e:
      raise e

  #################################################################
  # consume the data on our pub socket
  #
  # consume info using the ZMQ pub socket
  #
  ...
  # sending Subscriber id, topic, data, timestamp at a minimum for our experimental
  # data collection. So anyway we will need to do the necessary serialization.
  #
  # This part is left as an exercise.
  #################################################################
  def consume (self, id, topiclist, addr, port):
    try:
      self.logger.debug ("SubscriberMW::consume - topic " + str(topiclist))

      # subscribe to all info matching topic filter
      for topic in topiclist:
        self.sub.setsockopt (zmq.SUBSCRIBE, bytes(topic, "utf-8"))

      self.logger.debug ("SubscriberMW::listening on - " + "tcp://" + str(addr) + ":" + str(port))
      self.sub.connect("tcp://" + str(addr) + ":" + str(port))

      data = self.sub.recv()

      self.logger.info (data.decode().replace(':',','))
    except Exception as e:
      raise e
